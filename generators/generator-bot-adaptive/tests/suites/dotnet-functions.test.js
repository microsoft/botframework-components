...
it (`should create file ${path.join(botName, 'ActivitySerializationSettings.cs')}`, function () {
  const filePath = path.join(botName, 'ActivitySerializationSettings.cs');
  const content = reader.getTemplateFileContent(path.join('dotnet', 'functions', 'ActivitySerializationSettings.cs'), {
    botName,
  });
...
it(`should create file ${path.join(botName, `${botName}.csproj`)}`, function () {
  ...
  const content = reader.getTemplateFileContent(path.join('dotnet', 'functions', 'botName.csproj'), {
    botName,
    packageReferences: '',
  });
...
it (`should create file ${path.join(botName, 'host.json')}`, function () {
  ...
  const content = reader.getFileContent(path.join('dotnet', 'functions', 'host.json'));

  runResult.assertFileContent(filePath, content);
});

it (`should create file ${path.join(botName, 'local.settings.json')}`, function () {
  const filePath = path.join(botName, 'local.settings.json');
  const content = reader.getFileContent(path.join('dotnet', 'functions', 'local.settings.json'));
...
it (`should create file ${path.join(botName, 'Startup.cs')}`, function () {
  ...
  const content = reader.getTemplateFileContent(path.join('dotnet', 'functions', 'Startup.cs'), {
    botName,
...
it (`should create file ${path.join(botName, 'Properties', 'serviceDependencies.json')}`, function () {
  const filePath = path.join(botName, 'Properties', 'serviceDependencies.json');
  const content = reader.getFileContent(path.join('dotnet', 'functions', 'Properties', 'serviceDependencies.json'));

  runResult.assertFileContent(filePath, content);
});

it (`should create file ${path.join(botName, 'Properties', 'serviceDependencies.local.json')}`, function () {
  const filePath = path.join(botName, 'Properties', 'serviceDependencies.local.json');
  const content = reader.getFileContent(path.join('dotnet', 'functions', 'Properties', 'serviceDependencies.local.json'));

  runResult.assertFileContent(filePath, content);
});

it (`should create file ${path.join(botName, 'Triggers', 'MessagesTrigger.cs')}`, function () {
  const filePath = path.join(botName, 'Triggers', 'MessagesTrigger.cs');
  const content = reader.getTemplateFileContent(path.join('dotnet', 'functions', 'Triggers', 'MessagesTrigger.cs'), {
    botName,
  });
...
it (`should create file ${path.join(botName, 'Triggers', 'SkillsTrigger.cs')}`, function () {
  const filePath = path.join(botName, 'Triggers', 'SkillsTrigger.cs');
  const content = reader.getTemplateFileContent(path.join('dotnet', 'functions', 'Triggers', 'SkillsTrigger.cs'), {
    botName,
  });
...
it (`should create file ${path.join(botName, 'Triggers', 'StaticFilesTrigger.cs')}`, function () {
  const filePath = path.join(botName, 'Triggers', 'StaticFilesTrigger.cs');
  const content = reader.getTemplateFileContent(path.join('dotnet', 'functions', 'Triggers', 'StaticFilesTrigger.cs'), {
    botName,
  });
