...
export interface GetTeamDetailsConfiguration extends DialogConfiguration {
...
export class GetTeamDetails
  extends Dialog
  implements GetTeamDetailsConfiguration {
  /**
   * Class identifier.
   */
   ...
   */
  public teamId: StringExpression = new StringExpression(
    '=turn.activity.channelData.team.id'
  );

  public getConverter(
    property: keyof GetTeamDetailsConfiguration
  ...
  public async beginDialog(
    ...
    if (dc.context.activity.channelId !== Channels.Msteams) {
      throw new Error(
        `${GetTeamDetails.$kind} works only on the Teams channel.`
      );
    }

    const teamId = getValue(dc, this.teamId);

    const result = await TeamsInfo.getTeamDetails(dc.context, teamId);
