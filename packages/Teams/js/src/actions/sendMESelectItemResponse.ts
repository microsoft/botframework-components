...
export interface SendMESelectItemResponseConfiguration
...
export class SendMESelectItemResponse
  ...
  public getConverter(
    property: keyof SendMESelectItemResponseConfiguration
  ...
  public async beginDialog(
    ...
    if (!this.card) {
      throw new Error(
        `A valid card is required for ${SendMESelectItemResponse.$kind}.`
      );
    }
    ...
    if (!attachment) {
      throw new Error(
        `Invalid activity. An attachment is required for ${SendMESelectItemResponse.$kind}.`
      );
    }
    ...
    const invokeResponse = this.createMessagingExtensionInvokeResponseActivity(
      dc,
      response
    );
