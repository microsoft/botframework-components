...
export interface GetMeetingParticipantConfiguration
 ...
 */
export class GetMeetingParticipant
  extends Dialog
  implements GetMeetingParticipantConfiguration {
  /**
   * Class identifier.
   */
   ...
   */
  public participantId = new StringExpression(
    '=turn.activity.from.aadObjectId'
  );
   ...
   */
  public tenantId = new StringExpression(
    '=turn.activity.channelData.tenant.id'
  );

  public getConverter(
    property: keyof GetMeetingParticipantConfiguration
  ...
  public async beginDialog(
    ...
    if (dc.context.activity.channelId !== Channels.Msteams) {
      throw new Error(
        `${GetMeetingParticipant.$kind} works only on the Teams channel.`
      );
    }
    ...
    const result = await TeamsInfo.getMeetingParticipant(
      dc.context,
      meetingId,
      participantId,
      tenantId
