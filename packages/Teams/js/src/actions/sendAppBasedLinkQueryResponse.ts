...
export interface SendAppBasedLinkQueryResponseConfiguration
...
export class SendAppBasedLinkQueryResponse
  ...
  public getConverter(
    property: keyof SendAppBasedLinkQueryResponseConfiguration
  ...
  public async beginDialog(
    ...
    if (!this.card) {
      throw new Error(
        `An activity with attachments is required for ${SendAppBasedLinkQueryResponse.$kind}.`
      );
    }
