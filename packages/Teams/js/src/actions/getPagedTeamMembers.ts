 ...
 */
export class GetPagedTeamMembers
  extends Dialog
  implements GetPagedTeamMembersConfiguration {
  /**
   * Class identifier.
   */
  ...
  public getConverter(
    property: keyof GetPagedTeamMembersConfiguration
  ...
  public async beginDialog(
    ...
    if (dc.context.activity.channelId !== Channels.Msteams) {
      throw new Error(
        `${GetPagedTeamMembers.$kind} works only on the Teams channel.`
      );
    }
    ...
    const result = await TeamsInfo.getPagedTeamMembers(
      dc.context,
      teamId,
      pageSize,
      continuationToken
