...
export interface SendMEBotMessagePreviewResponseConfiguration
...
export class SendMEBotMessagePreviewResponse
  ...
  public getConverter(
    property: keyof SendMEBotMessagePreviewResponseConfiguration
  ...
  public async beginDialog(
    ...
    if (!this.card) {
      throw new Error(
        `A valid Card is required for ${SendMEBotMessagePreviewResponse.$kind}.`
      );
    }
    ...
    const invokeResponse = BaseTeamsCacheInfoResponseDialog.createInvokeResponseActivity(
      response
    );
