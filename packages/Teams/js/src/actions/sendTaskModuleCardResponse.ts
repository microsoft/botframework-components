...
export interface SendTaskModuleCardResponseConfiguration
...
export class SendTaskModuleCardResponse
  ...
  public getConverter(
    property: keyof SendTaskModuleCardResponseConfiguration
  ...
  public async beginDialog(
    ...
    if (!this.card) {
      throw new Error(
        `A valid Card is required for ${SendTaskModuleCardResponse.$kind}.`
      );
    }
    ...
    if (!attachment) {
      throw new Error(
        `Invalid activity. An attachment is required for ${SendTaskModuleCardResponse.$kind}.`
      );
    }
    ...
    const responseActivity = BaseTeamsCacheInfoResponseDialog.createInvokeResponseActivity(
      response
    );
