...
export abstract class BaseAuthResponseDialog
  ...
  public async beginDialog(
    ...
    const tokenResponse = await BaseAuthResponseDialog.getUserToken(
      dc,
      connectionName
    );
    if (tokenResponse) {
  ...
  private async createOAuthInvokeResponseActivityFromTitleAndConnectionName(
    ...
    const signInLink = await dc.context.adapter.getSignInLink(
      dc.context,
      connectionName
    );
  ...
  private async createOAuthInvokeResponseActivityFromTitleAndSignInLink(
    ...
    return this.createOAuthInvokeResponseActivityFromCardAction(
      dc,
      signInAction
    );
  }
  ...
  private static async getUserToken(
    ...
    return dc.context.adapter.getUserToken(
      dc.context,
      connectionName,
      magicCode
    );
