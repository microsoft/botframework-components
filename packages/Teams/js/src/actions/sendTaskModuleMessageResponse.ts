...
export interface SendTaskModuleMessageResponseConfiguration
...
export class SendTaskModuleMessageResponse
  ...
  public getConverter(
    property: keyof SendTaskModuleMessageResponseConfiguration
  ...
  public async beginDialog(
    ...
    if (!message) {
      ...
      if (languageGenerator) {
        const lgStringResult = await languageGenerator.generate(
          dc,
          message,
          dc.state
        );
