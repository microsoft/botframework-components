...
namespace Microsoft.Bot.Components.Recognizers
{
    ...
    public class CustomQuestionAnsweringRecognizer : Recognizer
    {
        ...
        /// </summary>
        [JsonProperty("$kind")]
        public const string Kind = "Microsoft.Bot.Components.Recognizers";
        ...
        public override async Task<RecognizerResult> RecognizeAsync(DialogContext dialogContext, Activity activity, CancellationToken cancellationToken, Dictionary<string, string> telemetryProperties, Dictionary<string, double> telemetryMetrics)
        {
        ...
        protected virtual Task<IQnAMakerClient> GetQnAMakerClientAsync(DialogContext dc)
        {
            ...
            var (kbId, error3) = ProjectName.TryGetValue(dc.State);
            ...
            var endpoint = new QnAMakerEndpoint
            {
                ...
                KnowledgeBaseId = kbId ?? throw new InvalidOperationException($"Unable to get a value for {nameof(ProjectName)} from state. {error3}"),
                QnAServiceType = Microsoft.Bot.Builder.AI.QnA.Models.ServiceType.Language
            };
            ...
            return Task.FromResult<IQnAMakerClient>(new CustomQuestionAnswering(endpoint, options, httpClient, TelemetryClient, LogPersonalInformation.GetValue(dc.State)));
        }
