    ...
    public class OneNoteService : ITaskService
    {
        ...
        public async Task<ITaskService> InitAsync(string token, Dictionary<string, string> pageIds, HttpClient client = null)
        {
            try
            {
                ...
                var notebookId = await GetOrCreateNotebookAsync("Task Notebook");
                var sectionId = await GetOrCreateSectionAsync(notebookId, "Task Section");
                var pageId = await GetOrCreatePageAsync(sectionId, "To Do");
                pageIds.Add("To Do", pageId.Item1);
        ...
        /// </summary>
        /// <param name="listType">Task list type.</param>
        /// <param name="taskContent">The task text.</param>
        /// <returns>Ture if succeed.</returns>
        public async Task<bool> AddTaskAsync(string listType, string taskContent)
        {
            try
            {
                ...
                var httpRequestMessage = ServiceHelper.GenerateAddToDoHttpRequest(taskContent, todoContent, pageContentUrl.ContentUrl);
        ...
        public async Task<string> GetTaskWebLink()
        {
            ...
            var onenoteNotebook = await GetOneNoteNotebookAsync($"{notebooksUrl}?filter=name eq 'Task Notebook'");
