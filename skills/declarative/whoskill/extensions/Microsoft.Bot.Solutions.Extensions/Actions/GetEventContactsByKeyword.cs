...
{
    public class GetEventContactsByKeyword : Dialog
    {
        [JsonProperty("$kind")]
        public const string DeclarativeType = "Microsoft.Graph.Who.GetEventContactsByKeyword";

        [JsonConstructor]
        public GetEventContactsByKeyword([CallerFilePath] string callerPath = "", [CallerLineNumber] int callerLine = 0)
            : base()
        {
        ...
        public override async Task<DialogTurnResult> BeginDialogAsync(DialogContext dc, object options = null, CancellationToken cancellationToken = default)
        {
            ...
            var eventResult = await GraphService.GetEvent(token, keywordProperty, top);
            ...
            await dc.Context.TraceActivityAsync(nameof(GetEventContactsByKeyword), eventContacts, valueType: DeclarativeType, label: this.Id).ConfigureAwait(false);
