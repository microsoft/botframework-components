...
namespace ITSMSkill.Dialogs.Teams.SubscriptionTaskModule
{
    ...
    public class CreateSubscriptionTeamsTaskModule : ITeamsTaskModuleHandler<TaskModuleContinueResponse>
    {
        ...
        public CreateSubscriptionTeamsTaskModule(
        ...
        public async Task<TaskModuleContinueResponse> OnTeamsTaskModuleSubmitAsync(ITurnContext context, CancellationToken cancellationToken)
        {
            ...
            if (isDataObject)
            {
                ...
                // Get filterUrgency
                var filterCondition = dataObject.GetValue("FilterCondition");
                ...
                if (filterCondition.Value<string>().Contains("1"))
                {
                    filterList.Add(conditions[0]);
                }

                if (filterCondition.Value<string>().Contains("2"))
                {
                    filterList.Add(conditions[1]);
                }

                if (filterCondition.Value<string>().Contains("3"))
                {
                    filterList.Add(conditions[2]);
                }

                if (filterCondition.Value<string>().Contains("4"))
                {
                ...
                if (isNewSubscription)
                {
                    ...
                    var result = await management.CreateSubscriptionBusinessRule(filterList, filterName.Value<string>(), notificationNameSpace.Value<string>(), postNotificationAPIName.Value<string>());
