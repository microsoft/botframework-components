...
using RideshareSkill.Extensions;
using RideshareSkill.Models;
using RideshareSkill.Services;

namespace RideshareSkill.Dialogs
{
    ...
    public class MainDialog : ComponentDialog
    {
        ...
        protected override async Task<DialogTurnResult> OnBeginDialogAsync(DialogContext innerDc, object options, CancellationToken cancellationToken = default)
        {
            ...
            if (activity.Type == ActivityTypes.Message && !string.IsNullOrEmpty(activity.Text))
            {
                ...
                var skillResult = await localizedServices.LuisServices["RideshareSkill"].RecognizeAsync<RideshareSkillLuis>(innerDc.Context, cancellationToken);
        ...
        private async Task<DialogTurnResult> RouteStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            if (activity.Type == ActivityTypes.Message && !string.IsNullOrEmpty(activity.Text))
            {
                ...
                localizedServices.LuisServices.TryGetValue("RideshareSkill", out var luisService);

                if (luisService != null)
                {
                    var result = stepContext.Context.TurnState.Get<RideshareSkillLuis>(StateProperties.SkillLuisResult);
                    var intent = result?.TopIntent().intent;

                    switch (intent)
                    {
                        case RideshareSkillLuis.Intent.Sample:
                            {
                                return await stepContext.BeginDialogAsync(_sampleDialog.Id);
                            }

                        case RideshareSkillLuis.Intent.None:
                        default:
                            {
