    ...
    public class MainDialog : ComponentDialog
    {
                ...
                if (generalScore > 0.5)
                {
                    switch (generalIntent)
                    {
                        case General.Intent.Cancel:
                            {
                                ...
                                await innerDc.Context.SendActivityAsync(_localeTemplateManager.GetResponse(RestaurantBookingSharedResponses.CancellingMessage));
                        ...
                        case General.Intent.Help:
                            {
                                await innerDc.Context.SendActivityAsync(_localeTemplateManager.GetResponse(RestaurantBookingMainResponses.HelpMessage));
        ...
        private async Task<DialogTurnResult> IntroStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            else
            {
                ...
                {
                    Prompt = stepContext.Options as Activity ?? _localeTemplateManager.GetResponse(RestaurantBookingMainResponses.FirstPromptMessage)
                };

                if (stepContext.Context.Activity.Type == ActivityTypes.ConversationUpdate)
                {
                    promptOptions.Prompt = _localeTemplateManager.GetResponse(RestaurantBookingMainResponses.WelcomeMessage);
                }
                    ...
                    case ReservationLuis.Intent.None:
                        {
                            ...
                            await stepContext.Context.SendActivityAsync(_localeTemplateManager.GetResponse(RestaurantBookingSharedResponses.DidntUnderstandMessage));
                            break;
                        }
        ...
        private async Task<DialogTurnResult> FinalStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            {
                return await stepContext.ReplaceDialogAsync(InitialDialogId, _localeTemplateManager.GetResponse(RestaurantBookingMainResponses.CompletedMessage), cancellationToken);
            }
        }
